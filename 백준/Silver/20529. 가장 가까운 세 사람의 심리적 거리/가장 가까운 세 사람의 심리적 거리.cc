...
//
//MBTI(Myers - Briggs Type Indicator)는 C.G.Jung의 심리유형론을 근거로 하여 Katharine Cook Briggs와 Isabel Briggs Myers가 
...
// 일반적으로 MBTI의 유형들은 각 분류를 나타내는 알파벳 한 글자씩을 따 네 글자로 표시하게 된다.모든 유형의 목록은 다음과 같다.
//
		...
		for (int i = 0; i < mbtis.size() - 2; ++i) {
			for (int j = i + 1; j < mbtis.size() - 1; ++j) {
				for (int k = j + 1; k < mbtis.size(); ++k) {
					int nowVal = 0;
					for (int t = 0; t < 4; ++t) {
						if (!(mbtis[i][t] == mbtis[j][t] && mbtis[i][t] == mbtis[k][t])) {
							nowVal += 2;
						}
					}
					if (result > nowVal)
