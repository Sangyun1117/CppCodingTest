...
// 문장의 맨 뒤(마지막 문자의 오른쪽), 또는 문장 중간 임의의 곳(모든 연속된 두 문자 사이)에 
// 위치할 수 있다.즉 길이가 L인 문자열이 현재 편집기에 입력되어 있으면, 
...
int main() {
	...
	list<char> nowStrList;
	for (char c : str) {
		nowStrList.push_back(c);
	}
	auto cursor = nowStrList.end();
	int M;
	cin >> M;
	...
	for (int i = 0; i < M; ++i) {
		...
		case 'P': {
			char tmp;
			cin >> tmp;
			nowStrList.insert(cursor, tmp);
			break;
		}
		case 'L':
			if (cursor != nowStrList.begin())
				cursor--;
			break;
		case 'D':
			if (cursor != nowStrList.end())
				cursor++;
			break;
		case 'B':
			if (cursor != nowStrList.begin()) {
				cursor--;
				cursor = nowStrList.erase(cursor);
			}
			break;
		default:
			break;
